		  ...
		  )
{
  ...
  if (layerPairIndex < numberOfLayerPairs)
  {
  	  	                       ...
  	  	                       i += gridDim.x * blockDim.x)
    {
      ...
      isOuterHitOfCell[outerLayerId]
                      [thisCell.get_outer_hit_id()].push_back_ts(&(thisCell));
    }
  }
  else
  {
		   ...
		   )
{
  ...
  if (layerPairIndex < numberOfLayerPairs)
  {
  	  	                       ...
  	  	                       i += gridDim.x * blockDim.x)
    {
      ...
      int    numberOfPossibleNeighbors = isOuterHitOfCell[innerLayerId]
                                                         [innerHitId].size();
      for (int j = 0; j < numberOfPossibleNeighbors; ++j)
      {
  	GPUCACell* otherCell = isOuterHitOfCell[innerLayerId]
	                                       [innerHitId].m_data[j];
			 ...
			 )
{
  		                         ...
  		                         i += gridDim.x * blockDim.x)
  {
    ...
    cells[lastLayerPairIndex][i].find_ntuplets(
                                               foundNtuplets, 
					       stack, 
					       minHitsPerNtuplet
					      );
...
// and then we move the containers of the hits
 ...
 for (unsigned int i = 0; i < theNumberOfLayers; ++i)
 {
  cudaMalloc  	(
 	      	 ...
 	      	 hitsOnLayer[i]->size() * sizeof(GPUSimpleVector<80, GPUCACell*> )
	      	);
  cudaMemset  	(
              	 host_isOuterHitOfCell[i], 
		 0,
 ...
 kernel_create       <<<numberOfBlocks,256>>>(
                                              gpu_doublets    	  ,
					      gpu_layerHits   	  ,
					      theCells        	  ,
					      isOuterHitOfCell    ,
 ...
 kernel_connect      <<<numberOfBlocks,256>>>(
                                              gpu_doublets         ,
					      theCells             ,
					      isOuterHitOfCell     ,
					      thePtMin             ,
 ...
 kernel_find_ntuplets<<<numberOfBlocks,128>>>(
                                              gpu_doublets  	    	  ,
					      theCells      	    	  ,
					      foundNtuplets	    	  ,
					      gpu_externalLayerPairs 	  ,
