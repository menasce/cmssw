...
class CACell 
{
 public:
		    ...
		    ) const 
   {
     ...
     float distance_13_squared = (r1 - theOuterR) *
                                 (r1 - theOuterR) + 
				 (z1 - theOuterZ) *
				 (z1 - theOuterZ) ;
     float tan_12_13_half_mul_distance_13_squared = fabs(z1        * (theInnerR - theOuterR) + 
			    ...
			    ) const 
   {
     ...
     auto minimumOfIntersectionRange = (radius - region_origin_radius) *
   ...
   // if there is at least one left neighbor with the same state (friend), 
   // the state has to be increased by 1.
		    ...
		    ) const 
   {
     ...
     else 
     {
       ...
       for (unsigned int i = 0; i < numberOfOuterNeighbors; ++i) 
       {
         ...
         theOuterNeighbors[i]->findNtuplets(
	                                    foundNtuplets, 
					    tmpNtuplet, 
					    minHitsPerNtuplet
					   );
