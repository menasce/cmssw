...
class GPUCACell
{
 public:
  ...
  __device__
  void init(
            const GPULayerDoublets * doublets   , 
		     ...
		     ) const
  {
    ...
    float distance_13_squared = radius_diff*radius_diff + (z1 - theOuterZ)*
                                                          (z1 - theOuterZ);
    float pMin = ptmin*sqrt(distance_13_squared); //this needs to be divided by radius_diff later
    float tan_12_13_half_mul_distance_13_squared = fabs(
                                                        z1        * (theInnerR - theOuterR) + 
                                                        theInnerZ * (theOuterR - r1       ) + 
							theOuterZ * (r1        - theInnerR)
						       ) ;
    return tan_12_13_half_mul_distance_13_squared * pMin <= thetaCut            * 
                                                            distance_13_squared * 
							    radius_diff;
  }
  //_______________________________________________
  __device__
  bool have_similar_curvature(
                              const GPUCACell * otherCell           ,
  		              const float 	ptmin               ,
			     ...
			     ) const
  {
   ...
   float distance_13_squared = (x1 - x3)*(x1 - x3) + 
                               (y1 - y3)*(y1 - y3);
   float tan_12_13_half_mul_distance_13_squared = fabs(
                                                       y1 * (x2 - x3) + 
						       y2 * (x3 - x1) + 
   ...
   if(tan_12_13_half_mul_distance_13_squared * ptmin <= 1.0e-4f*distance_13_squared)
   {
    ...
    auto centers_distance_squared 	     = (x_center - region_origin_x) *
				  	       ...
				  	       (y_center - region_origin_y) *
				  	       (y_center - region_origin_y) ;
    auto region_origin_radius_plus_tolerance = region_origin_radius + phiCut;
    auto minimumOfIntersectionRange 	     = (radius - region_origin_radius_plus_tolerance)*
